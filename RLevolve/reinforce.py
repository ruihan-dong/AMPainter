...
def train_agent(save_path, device, 
    ...
    for step in tqdm(range(n_steps)):
        ...
        for bn in range(batch_num):
            ...
            # Experience Replay
            # NOTICE: why 4?
            # WARNING: no adding exp_seqs and exp_score
            if experience_replay and len(experience) > 4:
                exp_seqs, exp_score, exp_agent_likelihood = experience.sample(4)
                exp_agent_likelihood = torch.tensor(exp_agent_likelihood).to(device)
        ...
        if mean_train_loss < best_loss:
            ...
            torch.save(agent.mutator.state_dict(), os.path.join(save_path, 'Agent.ckpt'))
...
if __name__ == "__main__":
    ...
    model, tokenizer = ankh.load_base_model(generation=True)   # load_base_model / load_large_model
    ...
    agent_path = './weights/ite8_step40_Agent.ckpt'

    t0 = time()
    train_agent(save_path = './outputs/amp200_step10_ite8_V1/',
                device = device, 
                model = model, 
                tokenizer = tokenizer, 
                ...
                batchs_seqs = initial_seq, n_steps = 10, iterations = 8,   # random200 use n_steps=20
