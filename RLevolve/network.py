...
class Agent():
    ...
    def sample(self, batch_seqs, iterations, batch_num):
        """
        ...
        base_scores = scoring_function(batch_seqs)

        # idnumber = list(range(batch_size))
        ini_dist = list(np.zeros(batch_size))
        batch_idx = list(np.repeat(batch_num * 128, batch_size))
        memory_idx = [(i+idx) for i, idx in enumerate(batch_idx)]
        ite_memory = pd.DataFrame({'ite': ini_dist, 'seqs': batch_seqs, 'dist': ini_dist, 'score': base_scores}, index=(memory_idx))
        ...
        for ites in range(iterations):
            ...
            distance = levenshtein(output_seqs, batch_seqs)
            endseq_idx = []
            for i, seq in enumerate(output_seqs):
                prev_seqs = ite_memory[ite_memory.index == i]['seqs'].values.tolist()
                # print('prev_seqs: ', prev_seqs)
                # base_score = base_scores[i]
                # score = output_score[i]
                # if (seq in prev_seqs) or (score <= base_score):
                if (seq in prev_seqs):
                    endseq_idx.append(i)

            ite_numbers = list(np.repeat(ites+1, batch_size))
            df = pd.DataFrame({'ite': ite_numbers, 'seqs': output_seqs, 'dist': distance, 'score': output_score}, index=(memory_idx))
        ...
        return sequences, log_probs, ite_memory
